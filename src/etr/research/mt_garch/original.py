...
"""
    https://github.com/EduardoRamosP/MultiTransformer/blob/master/MultiTransformer/1.%20Train/Source/5_MTL_GARCH.ipynb
"""

# Return calculation
def ReturnCalculation(Database, lag):
    dimension = Database.shape[0]
    dif = lag
...
def DatabaseGeneration(Database, Lag, LagSD):
    ...
    DailyReturnsOld = np.append(
        np.repeat(np.nan, 1), DailyReturns[0 : (DailyReturns.shape[0] - 1)]
    )
...
def GJR_GARCH_Model_Student(Data):
    ...
    For_CV_GJR_GARCH11 = np.array(
        res_GJR_GARCH11.forecast(horizon=1).variance.dropna()
    )[0][0]
...
def AR_Models(Data):
    ...
    AVGARCH, AVGARCH_Parameters, CV_AVGARCH, For_CV_AVGARCH = AVGARCH_Model_Student(
        Data
    )
    FIGARCH, FIGARCH_Parameters, CV_FIGARCH, For_CV_FIGARCH = FIGARCH_Model_Student(
        Data
    )
    return (
        GARCH_Parameters,
...
class TransformerBlock(tf.keras.layers.Layer):
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1):
        ...
        for i in range(self.Bagging):
            self.nb_dict["att{0}".format(i)] = MultiHeadSelfAttention(
                embed_dim, num_heads
            )
    ...
    def call(self, inputs, training):
        self.att_dict = {}
        for i in range(self.Bagging):
            self.att_dict["att{0}".format(i)] = self.nb_dict["att{0}".format(i)](
                tf.keras.layers.Dropout(0.1)(inputs)
            )
            if i == 0:
                self.att_dict["attn_output"] = (
                    self.att_dict["att{0}".format(i)] / self.Bagging
                )
            else:
                self.att_dict["attn_output"] = (
                    self.att_dict["attn_output"]
...
def Transformer_Model(Shape1, Shape2, HeadsAttention, Dropout, LearningRate):
    ...
    transformer_block_1 = TransformerBlock(
        embed_dim=16, num_heads=HeadsAttention, ff_dim=8, rate=Dropout
    )
...
def DatabaseGenerationForecast_AR(
    Database,
    Lag,
    LagSD,
    For_CV_GARCH,
    ...
    Index_Forecast = DatabaseGenerationForecast(Database, Lag, LagSD).index[
        (-LagSD + 1)
    ]
    XDataForecast = {
        "SD": Data_Forecast["SD"],
    ...
    return (
        pd.DataFrame([XDataForecast], index=[Index_Forecast]),
        Data_Forecast["DailyReturns"],
    )


# Transformed ANN-ARCH model forecast
def T_ANN_ARCH_Forecast(
    Database,
    Lag,
    LagSD,
    For_CV_GARCH,
    ...
    XDataForecastTotalScaled_T, Y_T = Transformer_Database(
        Timestep, XDataForecastTotalScaled, np.zeros(XDataForecastTotalScaled.shape[0])
    )
    TransformerPrediction = model.predict(XDataForecastTotalScaled_T)
    return (
        TransformerPrediction[-1][0],
...
def T_ANN_ARCH_Fit(
    ...
    if Data_AR.shape[0] != Data.shape[0]:
    ...
    XData_AR_Norm_T, YData_AR_Norm_T = Transformer_Database(
        Timestep, XData_AR_Norm, YData_AR
    )
    ...
    model.fit(
        XData_AR_Norm_T, YData_AR_Norm_T, epochs=Epochs, verbose=0, batch_size=BatchSize
    )
    tf.keras.backend.clear_session()
    Forecast, Date_Forecast, TrainPrediction, ReturnForecast = T_ANN_ARCH_Forecast(
        Database,
        Lag,
        LagSD,
        For_CV_GARCH,
    ...
    return {
        "T_ANN_ARCH_model": model,
        "Forecast_T_ANN_ARCH": Forecast,
