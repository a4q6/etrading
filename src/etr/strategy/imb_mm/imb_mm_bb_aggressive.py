...
class ImbMM_BB(StrategyBase):
    ...
    def __init__(
        ...
        # trading attributes
        self.entry_order: Dict[int, Order] = {+1: Order.null_order(), -1: Order.null_order()}
    ...
    async def on_message(self, msg: Dict):
        ...
        if dtype == "Order":
            # update order status
            side = msg["side"]
            if msg["order_id"] == self.entry_order[side].order_id:
                msg.pop("_data_type")
                self.entry_order[side] = Order(**msg)
                self.logger.info(f"Updated entry order status (order_id = {self.entry_order[side].order_id}, {self.entry_order[side].order_status})")
                if self.entry_order[side].order_status in [OrderStatus.Filled]:
                    self.logger.info(f"ENTRY LIMIT ORDER FILLED")
                    self.latest_execution = msg["timestamp"]
                    async with self.entry_order[side].lock:
    ...
    async def _place_entry_order(self, msg: Dict, dtype: str):
        ...
        imbalance_signal = sufficient_imb * imbalance_side  # -1, 0, +1
        ema_alert = np.sign(sum([np.sign(cep.ema_ret) * (abs(cep.ema_ret) > self.ema_threshold) for cep in self.ema.values()]))  # -1, 0, +1
        # if np.isnan(imbalance_signal):
        #     return

        if imbalance_signal == 0:
            # Place both sides if not live
            for side in [-1, +1]:
                new_price = self.pricing(side=side)
                o = self.entry_order[side]
                alert = ema_alert * side < 0
                # EMA alert => 刺さない & cancel
                # liveでない => 指す
                # live ^ 300ms経過 => 遠いなら指しなおす
                cancel_existing = False
                place_new = False
                if not o.is_live and not alert:
                    place_new = True
                    cancel_existing = False
                elif not o.is_live and alert:
                    place_new = False
                    cancel_existing = False
                elif o.is_live and alert:
                    place_new = False
                    cancel_existing = True
                elif o.is_live and not alert:
                    is_cur_price_far = abs(new_price / o.price - 1) * 1e4 > 0.1
                    place_new = is_cur_price_far
                    cancel_existing = True

                if cancel_existing and msg["timestamp"] > o.timestamp + datetime.timedelta(seconds=0.3):
                    if not o.is_locked:
                        async with o.lock:
                            self.logger.info(f"Cancel entry limit order (order_id = {o.order_id})")
                            self.entry_order[side] = await self.client.cancel_order(
                                o.order_id,
                            ...
                            place_new = self.entry_order[side].order_status not in (OrderStatus.Filled, OrderStatus.Partial)

                if msg["timestamp"] > o.timestamp + datetime.timedelta(seconds=0.3) and place_new:
                    if not self.entry_order[side].is_locked:
                        async with self.entry_order[side].lock:
                            self.logger.info(f"Place entry limit order @({side}, {new_price})")
                            self.entry_order[side] = await self.client.send_order(
                                msg["timestamp"], self.sym, side=side, price=new_price, amount=self.amount, order_type=OrderType.Limit,
                                src_type=dtype, src_timestamp=msg["timestamp"], src_id=msg["universal_id"], misc=f"entry; {imbalance_signal}; {ema_alert}")

        elif not np.isnan(imbalance_signal) and imbalance_signal != 0:
            # signalと逆サイドがliveならcancel
            # alertと逆サイドがliveならcancel
            # 順張り方向のOrderをupdate
            if self.entry_order[imbalance_signal * (-1)].is_live:
                # cancel -- imbalance signal
                side = imbalance_signal * (-1)
                o = self.entry_order[side]
                if not o.is_locked:
                    async with o.lock:
                        self.logger.info(f"Cancel entry limit order (order_id = {o.order_id})")
            ...
            if ema_alert != 0 and self.entry_order[ema_alert * (-1)].is_live:
                # cancel -- ema alert
                side = ema_alert * (-1)
                o = self.entry_order[side]
                if not o.is_locked:
                ...
                o = self.entry_order[side]
                new_price = self.pricing(side=imbalance_signal)
                is_cur_price_far = abs(new_price / o.price - 1) * 1e4 > 0.1
                if self.entry_order[imbalance_signal].is_live and is_cur_price_far:
                    # cancel
                    if not o.is_locked:
                        async with o.lock:
                            self.logger.info(f"Cancel entry limit order (order_id = {o.order_id})")
                ...
                if not self.entry_order[side].is_live:
                    # new
                    o = self.entry_order[side]
                    if not o.is_locked:
                        async with o.lock:
            ...
            # await self.client.cancel_all_orders(msg["timestamp"], sym=self.sym)
