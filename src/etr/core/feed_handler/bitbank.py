...
class BitBankSocketClient:
    def __init__(
        ...
        self.ticker_plant = AsyncBufferedLogger(logger_name="TP_" + self.__class__.__name__, log_dir=tp_file.as_posix())
    ...
    async def _process_message(self, raw_msg: str):
        ...
        if body["room_name"].startswith("transaction"):
            ...
            for trs in transactions:
                ...
                asyncio.create_task(self.ticker_plant.info(json.dumps(data.to_dict()))) # store
        ...
        if body["room_name"].startswith("depth"):
            ...
            if cur_book.misc[0] != "null":
                ...
                if self.rate[ccypair].mid_price != new_rate.mid_price:
                    ...
                    asyncio.create_task(self.ticker_plant.info(json.dumps(new_rate.to_dict())))  # store
